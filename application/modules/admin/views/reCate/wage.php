        ...
        <div class="col-md-6">
            ...
            <div class="panel panel-flat">
                ...
                <table class="table datatable-html">
                    ...
                    <tr>
                        <th>ID</th>
                        <th><?=$this->lang->line('wage'); ?></th>
                        <th class="text-center"><?=$this->lang->line('action'); ?></th>
                    </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($wage_vn as $key => $item) {?>
                            <tr>
                                <td><?=$item->id?></td>
                                <td><?=$item->name?></td>
        ...
        <div class="col-md-6">
            ...
            <div class="panel panel-flat">
                ...
                <table class="table datatable-html">
                    ...
                    <tbody>
                        <?php foreach ($wage_eng as $key => $item) {?>
                            <tr>
                                <td><?=$item->id?></td>
                                <td><?=$item->name?></td>
                                <td class="text-center">
...
<script type="text/javascript">
    function editTitle(id,lang){
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/reCate/wage_edit'); ?>",
            data:{id:id,lang:lang},
            success: function(result){
                var html        = '<form class="form-horizontal">';
                    html        +=  '<div class="form-group">';
                    html        +=      '<label for="Title" class="col-sm-2 control-label"><?=$this->lang->line('wage'); ?></label>';
                    html        +=      '<div class="col-sm-10">';
                    html        +=          '<input type="text" class="form-control" id="Title" placeholder="<?=$this->lang->line('wage'); ?>" value="'+result.name+'" required>';
    ...
    function delTitle(id,lang){
        if (confirm("are you ready?")) {
            $.ajax({
                dataType: "json",
                type:"POST",
                url:"<?=base_url('admin/reCate/wage_del'); ?>",
                data:{id:id,lang:lang},
                success: function(result){
                    if(result == 1){
    ...
    function saveTitle(){
        ...
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/reCate/wage_update'); ?>",
            ...
            <div class="modal-body">
                <form class="form-horizontal">
                    <div class="form-group">
                        <label for="Title" class="col-sm-2 control-label"><?=$this->lang->line('wage'); ?></label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" id="Title" placeholder="<?=$this->lang->line('wage'); ?>" />
                        </div>
                    </div>
                    <div class="form-group">
...
<script>
    function addDataTitle(){
        ...
        $.ajax({
            dataType: "json",
            type:"POST",
            url:"<?=base_url('admin/reCate/wage_add'); ?>",
