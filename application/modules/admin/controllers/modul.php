 ...
 */
	class Modul extends My_Controller{
		...
		public function index(){
			$data['title']             = 'Danh sách modules';
			$data['modul']					=	$this->uri->segment(2);
			$data['cate_building_vn']  			= $this->getCateBuilding(1);
			$data['cate_building_en']  			= $this->getCateBuilding(2);
			$data['content']          = 'modul/list';

			$data['modul_list']		= 'class="active"';
		...
		public function list_all(){
			...
			$param                     = array();
			$param['select']           = array('username','user_fullname','user_email','iduser','usergroup','user_activation');
			...
			$url    = base_url().'admin/users/list_all';

			$this->load->model('muser');
			$config = pagination($url,$per_page = 20,$total = $this->muser->get_total($param),$uri_segment = 4);
