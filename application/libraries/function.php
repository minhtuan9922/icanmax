...
// ham tra ve du lieu chua json_decode
if(!function_exists('returnData')){
	function returnData($url_file){
		if(file_exists($url_file)){
			$data	= file_get_contents($url_file);
...
function ckeditor($name, $value = ''){
	...
	$string .= '<script>
        ...
        \'save table contextmenu directionality emoticons template paste textcolor qrcode youtube \'
    ],
            toolbar: \'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image | print preview media fullpage | forecolor backcolor emoticons qrcode youtube \',
        });
</script>';
...
if(!function_exists('return_cus_type')){
	function return_cus_type($type){
		switch ($type) {
			case '1':
				return 'Candidate';
				break;
...
if(!function_exists('returnLangType')){
		function returnLangType($type){
			if(2 == $type){
				echo 'ENG';
			}else{
					echo "VN";
