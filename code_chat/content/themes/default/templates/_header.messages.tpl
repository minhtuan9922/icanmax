<li class="dropdown js_live-messages {if $user->_is_admin}is-admin{/if}">
    ...
    <div class="dropdown-menu dropdown-widget">
        <div class="dropdown-widget-header">
            {__("Tin nhắn")}
            <a class="pull-right flip flip flip text-link js_chat-start" href="{$system.system_url}/messages/new">{__("Tin nhắn mới")}</a>
        </div>
        <div class="dropdown-widget-body">
            <div class="js_scroller">
                ...
                <p class="text-center text-muted mt10">
                    {__("Không có tin nhắn nào")}
                </p>
                {/if}
            </div>
        </div>
        <a class="dropdown-widget-footer" href="{$system.system_url}/messages">{__("Xem tất cả")}</a>
    </div>
</li>