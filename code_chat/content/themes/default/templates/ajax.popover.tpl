    ...
    <div class="user-popover-content">
        <div class="user-card">
            ...
            <div class="user-card-info">
                ...
                <i data-toggle="tooltip" data-placement="top" title="{__("Hồ sơ xác minh")}" class="fa fa-check"></i>
                {/if}
                <div class="info">
                    <a href="{$system['system_url']}/{$profile['user_name']}/followers">{$profile['followers_count']} {__("theo dõi")}</a>
                </div>
            </div>
        </div>
            ...
            {if !is_empty($profile['user_work_title'])}
                {if $profile['user_id'] == $user->_data['user_id'] || $profile['user_privacy_work'] == "public" || $profile['we_friends']}
                    <div class="mb10">
                        ...
                        {$profile['user_work_title']} {__("tại")} <span class="text-link">{$profile['user_work_place']}</span>
                    </div>
                {/if}
            {/if}
            <!-- work -->
            ...
            {if !is_empty($profile['user_hometown'])}
                {if $profile['user_id'] == $user->_data['user_id'] || $profile['user_privacy_location'] == "public" || $profile['we_friends']}
                    <div class="mb10">
                        ...
                        {__("Từ")} <span class="text-link">{$profile['user_hometown']}</span>
                    </div>
                {/if}
            {/if}
            <!-- hometown -->
            ...
            {if $user->_data['user_id'] != $profile['user_id']}
                ...
                {else}
                    <div class="btn btn-success btn-sm js_friend-add" data-uid="{$profile['user_id']}">
                        <i class="fa fa-check fa-user-plus"></i> {__("Thêm bạn")}
                    </div>
                {/if}
    ...
    <div class="user-popover-content">
        <div class="user-card">
            ...
            <div class="user-card-info">
                ...
                <i data-toggle="tooltip" data-placement="top" title="{__("Hồ sơ xác minh")}" class="fa fa-check"></i>
                {/if}
                <div class="info">{$profile['page_likes']} {__("Đã thích")}</div>
            </div>
        </div>
        <div class="footer">
            {if $profile['i_like']}
