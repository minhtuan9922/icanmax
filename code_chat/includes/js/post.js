...
$(function() {
    ...
    $('body').on('keyup', '.js_publisher-scraber', function() {
        ...
        /* check if there is current (uploading|scrabing) process */
        if(publisher.data('uploading') || publisher.data('scrabing')) {
        	return;
        }
    ...
    $('body').on('click', '.js_publisher', function() {
        ...
        if(textarea.val() == "" && photos === undefined && link === undefined) {
            return;
        }
        _this.button('loading');
        $.post(api['posts/post'], {'handle': handle, 'id': id, 'message': textarea.val(), 'photos': JSON.stringify(photos), 'link': JSON.stringify(link), 'location':location.val(), 'privacy': privacy}, function(response) {
            if(response.callback) {
                _this.button('reset');
                eval(response.callback);
            } else {
