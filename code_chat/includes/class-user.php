...
class User {
    ...
    public function get_new_people($offset = 0, $random = false) {
        ...
        $new_people = array();
        if($random) {
            $get_new_people = $db->query(sprintf("SELECT * FROM users WHERE user_id NOT IN (%s) ORDER BY RAND() LIMIT %s, %s", $old_people_ids_list, secure($offset, 'int', false), secure($system['min_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        } else {
            $get_new_people = $db->query(sprintf("SELECT * FROM users WHERE user_id NOT IN (%s) LIMIT %s, %s", $old_people_ids_list, secure($offset, 'int', false), secure($system['min_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        }
    ...
    public function connect($do, $id) {
        ...
        switch ($do) {
            case 'block':
                /* check blocking */
                if($this->blocked($id)) {
                    throw new Exception(__("Bạn đã chặn người dùng này trước khi!"));
                }
            ...
            while($notification = $get_notifications->fetch_assoc()) {
                ...
                switch ($notification['action']) {
                    case 'friend':
                        ...
                        $notification['message'] = __("Chấp nhận yêu cầu kết bạn");
                        break;

                    case 'follow':
                        $notification['icon'] = "fa-rss";
                        $notification['url'] = $system['system_url'].'/'.$notification['user_name'];
                        $notification['message'] = __("Đang theo dõi bạn");
                        break;
                    ...
                    case 'comment':
                        ...
                        if($notification['node_type'] == "post") {
                            $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
                            $notification['message'] = __("Bình luận vào bài đăng của bạn");

                        } elseif ($notification['node_type'] == "photo") {
                            $notification['url'] = $system['system_url'].'/photos/'.$notification['node_url'];
                            $notification['message'] = __("Nhận xét về ảnh của bạn");
                        }
                        break;
                    ...
                    case 'share':
                        ...
                        $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
                        $notification['message'] = __("Chia sẻ bài viết của bạn");
                        break;
                    ...
                    case 'mention':
                        ...
                        if($notification['node_type'] == "post") {
                            $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
                            $notification['message'] = __("Nhắc đến bạn trong một bài đăng");

                        } elseif ($notification['node_type'] == "comment") {
                            $notification['url'] = $system['system_url'].'/posts/'.$notification['node_url'];
                            $notification['message'] = __("Nhắc đến bạn trong một bình luận");
    ...
    public function get_conversation($conversation_id) {
        ...
        /* decode html entity */
        //$conversation['message'] = html_entity_decode($conversation['message'], ENT_QUOTES);
        /* decode_emoji */
    ...
    public function publisher(array $args = array()) {
        ...
        if($args['handle'] == "page") {
            ...
            /* check if the group is valid & the viewer is the admin */
            $check_group = $db->query(sprintf("SELECT * FROM groups WHERE group_id = %s AND group_admin = %s", secure($args['id'], 'int'), secure($this->_data['user_id'], 'int') )) or _error(SQL_ERROR_THROWEN);
        ...
        $post['text'] = sanitize($args['message']);
    ...
    public function get_comments($node_id, $offset = 0, $is_post = true, $pass_privacy_check = true) {
        ...
        if($is_post) {
            ...
            /* get post comments */
            $get_comments = $db->query(sprintf("SELECT * FROM (SELECT posts_comments.*, users.user_name, users.user_fullname, users.user_gender, users.user_picture, pages.* FROM posts_comments Left JOIN users ON posts_comments.user_id = users.user_id AND posts_comments.user_type = 'user' Left JOIN pages ON posts_comments.user_id = pages.page_id AND posts_comments.user_type = 'page' WHERE posts_comments.node_type = 'post' AND posts_comments.node_id = %s ORDER BY posts_comments.comment_id DESC LIMIT %s, %s) comments ORDER BY comments.comment_id ASC", secure($node_id, 'int'), secure($offset, 'int', false), secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        } else {
            /* check privacy */
            if(!$pass_privacy_check) {
            ...
            /* get photo comments */
            $get_comments = $db->query(sprintf("SELECT * FROM (SELECT posts_comments.*, users.user_name, users.user_fullname, users.user_gender, users.user_picture, pages.* FROM posts_comments Left JOIN users ON posts_comments.user_id = users.user_id AND posts_comments.user_type = 'user' Left JOIN pages ON posts_comments.user_id = pages.page_id AND posts_comments.user_type = 'page' WHERE posts_comments.node_type = 'photo' AND posts_comments.node_id = %s ORDER BY posts_comments.comment_id DESC LIMIT %s, %s) comments ORDER BY comments.comment_id ASC", secure($node_id, 'int'), secure($offset, 'int', false), secure($system['max_results'], 'int', false) )) or _error(SQL_ERROR_THROWEN);
        }
    ...
    public function page_create($category, $title, $username, $description) {
        ...
        if(is_empty($category)) {
            throw new Exception(__("Bạn phải chọn danh mục hợp lệ cho trang của bạn"));
        }
        ...
        if(strlen($title) < 3) {
            throw new Exception(__("Tiêu đề trang phải có độ dài ít nhất 3 ký tự. Hãy thử một cái khác"));
        }
        /* validate username */
        if(is_empty($username)) {
            throw new Exception(__("Bạn phải nhập tên người dùng cho trang của bạn"));
        }
        if(!valid_username($username)) {
            throw new Exception(__("Vui lòng nhập một tên người dùng hợp lệ (a-z0-9_.)"));
        }
        if($this->check_username($username, 'page')) {
            throw new Exception(__("Xin lỗi , có vẻ như tên người dùng này")." <strong>".$username."</strong> ".__("thuộc về một trang hiện có"));
        }
        /* insert new page */
    ...
    public function page_edit($page_id, $category, $title, $username_new, $username_old, $description) {
        ...
        /* validate title */
        if(is_empty($title)) {
            throw new Exception(__("Bạn phải nhập tiêu đề cho trang của bạn"));
        }
        if(strlen($title) < 3) {
            throw new Exception(__("Tiêu đề trang phải có độ dài ít nhất 3 ký tự. Hãy thử một cái khác"));
        }
        ...
        if(strtolower($username_new) != strtolower($username_old)) {
            ...
            if($this->check_username($username_new, 'page')) {
                throw new Exception(__("Xin lỗi , có vẻ như tên người dùng này")." <strong>".$username_new."</strong> ".__("thuộc về một trang hiện có"));
            }
        }
        /* update the page */
    ...
    public function group_create($title, $username, $description) {
        ...
        /* validate title */
        if(is_empty($title)) {
            throw new Exception(__("Bạn phải nhập tiêu đề cho nhóm của bạn"));
        }
        if(strlen($title) < 3) {
            throw new Exception(__("Tiêu đề trang phải có độ dài ít nhất 3 ký tự. Hãy thử một cái khác"));
        }
        /* validate username */
        if(is_empty($username)) {
            throw new Exception(__("Bạn phải nhập tên người dùng cho nhóm của bạn"));
        }
        if(!valid_username($username)) {
            throw new Exception(__("Vui lòng nhập một tên người dùng hợp lệ (a-z0-9_.)"));
        }
        if($this->check_username($username, 'group')) {
            throw new Exception(__("Xin lỗi , có vẻ như tên người dùng này")." <strong>".$username."</strong> ".__("thuộc về một nhóm hiện có"));
        }
        /* insert new group */
    ...
    public function group_edit($group_id, $title, $username_new, $username_old, $description) {
        ...
        if(strtolower($username_new) != strtolower($username_old)) {
            ...
            if($this->check_username($username_new, 'group')) {
                throw new Exception(__("Xin lỗi , có vẻ như tên người dùng này")." <strong>".$username_new."</strong> ".__("thuộc về một nhóm hiện có"));
            }
        }
        /* update the group */
        ...
        switch ($args['edit']) {
            case 'username':
                ...
                if(strtolower($args['username']) != strtolower($this->_data['user_name'])) {
                    ...
                    if($this->check_username($args['username'])) {
                        throw new Exception(__("Tên đăng nhập")." <strong>".$args['username']."</strong> ".__("đã có người sử dụng"));
                    }
                    /* update user */
            ...
            case 'email':
                ...
                if($args['email'] != $this->_data['user_email']) {
                    if(!valid_email($args['email'])) {
                        throw new Exception(__("Email bạn nhập không hợp lệ"));
                    }
                    if($this->check_email($args['email'])) {
                        throw new Exception(__("Email ")." <strong>".$args['email']."</strong> ".__("đã có người sử dụng"));
                    }
                    ...
                    $subject = __("Chỉ cần thêm một bước nữa để bắt đầu")." ".$system['system_title'];
                    ...
                    if($system['email_smtp_enabled']) {
                        ...
                        if(!$mail->send()) {
                            ...
                            if(!send_email($args['email'], $subject, $body)) {
                                throw new Exception(__("Email kích hoạt không thể gửi được. Nhưng bạn có thể đăng nhập ngay bây giờ"));
                            }
                        }
                    } else {
                        /* send using mail() */
            ...
            case 'password':
                ...
                if(is_empty($args['current']) || is_empty($args['new']) || is_empty($args['confirm'])) {
                    throw new Exception(__("Bạn phải điền vào tất cả ô trống"));
                }
                /* validate current password */
                if(md5($args['current']) != $this->_data['user_password']) {
                    throw new Exception(__("Mật khẩu hiện tại của bạn là không chính xác"));
                }
                /* validate new password */
                if($args['new'] != $args['confirm']) {
                    throw new Exception(__("Mật khẩu của bạn không phù hợp"));
                }
                if(strlen($args['new']) < 6) {
                    throw new Exception(__("Mật khẩu của bạn phải có độ dài ít nhất 6 ký tự. Hãy thử một cái khác"));
                }
                /* update user */
            ...
            case 'basic':
                ...
                if(is_empty($args['fullname'])) {
                    throw new Exception(__("Bạn phải nhập tên của bạn"));
                }
                if(!valid_name($args['fullname'])) {
                    throw new Exception(__("Tên của bạn chứa các ký tự không hợp lệ"));
                }
                if(strlen($args['fullname']) < 3) {
                    throw new Exception(__("Tên của bạn phải dài ít nhất 3 ký tự. Hãy thử một cái khác"));
                }
                /* validate gender */
                if($args['gender'] == "none") {
                    throw new Exception(__("Vui lòng chọn một trong hai nam hoặc nữ"));
                }
                $args['gender'] = ($args['gender'] == "M")? "M" : "F";
                /* validate birthdate */
                if(!in_array($args['birth_month'], range(1, 12))) {
                    throw new Exception(__("Vui lòng chọn một tháng sinh hợp lệ"));
                }
                if(!in_array($args['birth_day'], range(1, 31))) {
                    throw new Exception(__("Vui lòng chọn ngày sinh hợp lệ"));
                }
                if(!in_array($args['birth_year'], range(1905, 2015))) {
                    throw new Exception(__("Vui lòng chọn năm sinh hợp lệ"));
                }
    ...
    public function sign_up($full_name, $username, $email, $password, $gender, $recaptcha_response) {
        ...
        if($this->check_username($username)) {
            throw new Exception(__("Xin lỗi , có vẻ như")." <strong>".$username."</strong> ".__("thuộc về tài khoản hiện có"));
        }
        if(!valid_email($email)) {
            throw new Exception(__("Vui lòng nhập một địa chỉ email hợp lệ"));
        }
        if($this->check_email($email)) {
            throw new Exception(__("Xin lỗi , có vẻ như")." <strong>".$email."</strong> ".__("thuộc về tài khoản hiện có"));
        }
        ...
        if($system['reCAPTCHA_enabled']) {
        	...
        	if (!$resp->isSuccess()) {
        		throw new Exception(__("Việc kiểm tra bảo mật là không chính xác . Vui lòng thử lại"));
        	}
        }
        /* generate user token */
        ...
        if($system['email_send_activation']) {
            ...
            $body .= "\r\n\r\n".__("Để hoàn tất quá trình đăng ký, xin vui lòng đi theo liên kết này:");
    ...
    public function sign_in($username_email, $password, $remember = false) {
        ...
        if(valid_email($username_email)) {
            if(!$this->check_email($username_email)) {
                throw new Exception(__("Các email bạn nhập không thuộc về bất kỳ tài khoản"));
            }
            $field = "user_email";
        }else {
            if(!valid_username($username_email)) {
                throw new Exception(__("Vui lòng nhập một địa chỉ email hợp lệ hoặc tên người dùng"));
            }
            if(!$this->check_username($username_email)) {
                throw new Exception(__("Tên người dùng bạn nhập vào không thuộc về bất kỳ tài khoản"));
            }
            $field = "user_name";
        }
        // get user
        ...
        if($get_user->num_rows == 0) {
            throw new Exception("<p><strong>".__("Xin vui lòng nhập lại mật khẩu của bạn")."</strong></p><p>".__("Mật khẩu bạn đã nhập không đúng").". ".__("Nếu bạn quên mật khẩu của bạn?")." <a href='".$system['system_url']."/reset'>".__("Yêu cầu mới")."</a></p>");
        }
        $user = $get_user->fetch_assoc();
        /* set cookies */
    ...
    public function socail_register($full_name, $username, $email, $password, $gender, $avatar, $provider) {
        ...
        if(!valid_username($username)) {
            throw new Exception(__("Vui lòng nhập một tên đăng nhập hợp lệ (a-z0-9_.)"));
        }
        if($this->check_username($username)) {
            throw new Exception(__("Xin lỗi , có vẻ như")." <strong>".$username."</strong> ".__("thuộc về tài khoản hiện có"));
        }
    ...
    public function forget_password($email, $recaptcha_response) {
        ...
        /* send reset email */
        /* prepare reset email */
        $subject = __("Quên mật khẩu mã khóa kích hoạt!");
        $body  = __("Hi")." ".$email.",";
        $body .= "\r\n\r\n".__("Để hoàn tất quá trình thiết lập lại mật khẩu, hãy sao chép mã token này :");
        $body .= "\r\n\r\n".__("Token:")." ".$reset_key;
        $body .= "\r\n\r".$system['system_title']." ".__("Team");
        /* send email */
        if($system['email_smtp_enabled']) {
            ...
            if(!$mail->send()) {
                /* send using mail() */
                if(!send_email($email, $subject, $body)) {
                    throw new Exception(__("Kích hoạt email quan trọng không thể được gửi !"));
                }
            }
        } else {
            /* send using mail() */
    ...
    public function forget_password_confirm($email, $reset_key) {
        global $db;
        if(!valid_email($email)) {
        	throw new Exception(__("Mã số không hợp lệ, vui lòng thử lại."));
        }
        /* check reset key */
    ...
    public function forget_password_reset($email, $reset_key, $password, $confirm) {
        global $db;
        if(!valid_email($email)) {
        	throw new Exception(__("Email không hợp lệ, vui lòng thử lại"));
        }
        /* check reset key */
        $check_key = $db->query(sprintf("SELECT user_reset_key FROM users WHERE user_email = %s AND user_reset_key = %s AND user_reseted = '1'", secure($email), secure($reset_key))) or _error(SQL_ERROR_THROWEN);
        if($check_key->num_rows == 0) {
        ...
        if($password !== $confirm) {
            throw new Exception(__("Mật khẩu của bạn không phù hợp. Hãy thử một cái khác"));
        }
    ...
    public function activation_email_resend() {
        ...
        if($system['email_smtp_enabled']) {
            ...
            /* send using mail() */
            if(!send_email($this->_data['user_email'], $subject, $body)) {
                throw new Exception(__("Email kích hoạt không thể gửi được."));
            }
        }
    }
    ...
    public function activation_email_reset($email) {
        ...
        if($system['email_smtp_enabled']) {
            ...
            if(!$mail->send()) {
                /* send using mail() */
                if(!send_email($email, $subject, $body)) {
                    throw new Exception(__("Email kích hoạt không thể gửi được. Nhưng bạn có thể đăng nhập lúc"));
                }
            }
        } else {
            /* send using mail() */
